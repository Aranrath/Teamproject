...
public class OptionsView extends GridPane {
	...
	private void buildView() {
		...
		addTopicButton.setOnAction((event)-> {
            ...
            stage.setScene(new Scene(new EditTopicView(topics, this, presenter), 450, 450));
            stage.show();
		});
		addPOButton.setOnAction((event)-> {
			Stage stage = new Stage();
            stage.setTitle("Neue PO");
            stage.setScene(new Scene(new EditPOView(pos, subjects, this, presenter), 450, 450));
            stage.show();
		});
		addSubjectButton.setOnAction((event)-> {
			Stage stage = new Stage();
            stage.setTitle("Neues Modul");
            stage.setScene(new Scene(new EditSubjectView(subjects,this, presenter), 450, 450));
            stage.show();
		});
	}
